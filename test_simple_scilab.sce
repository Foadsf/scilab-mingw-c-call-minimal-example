// Test for the absolute simplest Scilab-compatible DLL
function test_simple_scilab()
    // Path to the DLL
    dll_path = pwd() + filesep() + "simple_scilab" + getdynlibext();
    ...
    // Try to link - attempting multiple possible function name formats
    try
        // Link with explicit function name list
        lib_id = link(dll_path);
        mprintf("Link successful! Library ID: %d\n", lib_id);
        
        // Get function names from library
        mprintf("Attempting to list functions in the library...\n");
        try
            [funcs, native_names] = libraryinfo();
            mprintf("Library functions: \n");
            disp(funcs);
            mprintf("Native names: \n");
            disp(native_names);
        catch
        ...
        function_names = ["sci_test", "C2F(sci_test)"];
